...
def checkGasBalances():
    ...
    for chain in chains:
        ...
        try:
            ...
            gas_info = f"Gas amount on {chain}: {gas_amount}\n"
...
def checkMiniChefBalances():
    ...
    for chain in chains:
        ...
        try:
            ...
            miniChef_info = f"SYN balance on {chain}: {miniChef_amount} \n"
...
def checkExecutorBalances():
    ...
    for chain in chains:
        rpc_url = chains[chain]['url']
        contract_address = "0x230A1AC45690B9Ae1176389434610B9526d2f21b"

        try:
            response = make_rpc_request(rpc_url, "eth_getBalance", [contract_address, "latest"])
            gas_amount = round(int(response["result"], 16)/(10**18),2)
            gas_info = f"Executor Gas on {chain}: {gas_amount}\n"
            output += gas_info  # Append the gas info to the output string
...
def checkCCTPBalances():
    ...
    # Iterate over each chain
    for chain in chains:
        if chains[chain]['cctp'] is not None:
            rpc_url = chains[chain]['url']
